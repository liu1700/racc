...
pub async fn get_project_costs(worktree_path: String) -> Result<ProjectCosts, CoreError> {
    let home = home_dir().ok_or_else(|| CoreError::Other("Could not find home directory".into()))?;
...
pub async fn get_global_costs() -> Result<ProjectCosts, CoreError> {
    let home = home_dir().ok_or_else(|| CoreError::Other("Could not find home directory".into()))?;
