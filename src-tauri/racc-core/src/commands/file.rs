...
fn resolve_base_path(
    ...
    if let Some(sid) = session_id {
        ...
        match result {
            ...
            Err(e) => Err(CoreError::NotFound(format!("Session not found: {}", e))),
        }
        ...
        Err(CoreError::Other(
            "Either session_id or repo_id must be provided".to_string(),
        ))
    }
}

/// Validate that a file path is within the allowed base directory (prevent path traversal)
fn validate_path(base: &Path, relative: &str) -> Result<PathBuf, CoreError> {
    let full = base.join(relative);
    let canonical = full
        .canonicalize()
        .map_err(|e| CoreError::NotFound(format!("File not found: {}", e)))?;
    let base_canonical = base
        .canonicalize()
        .map_err(|e| CoreError::Other(format!("Base path invalid: {}", e)))?;
...
pub fn read_file_core(
    ...
    let text = String::from_utf8(bytes)
        .map_err(|_| CoreError::Other("File encoding not supported (not UTF-8)".to_string()))?;
